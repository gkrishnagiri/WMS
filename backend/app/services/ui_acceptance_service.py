...
UI_TEST_CATALOG: list[dict[str, Any]] = [
    ...
    {"suite_code": "AI_COSTING_AND_SMOKE_TEST_VALIDATION", "title": "AI Costing and Smoke Test Validation", "description": "Validate OpenAI catalog, local pricing, usage metering, guardrails, and disabled-by-default smoke controls.", "experience": "agentic,full,operations", "sort_order": 75, "cases": [_case("AI_COSTING_SMOKE_FLOW", "Costing and smoke-test flow", "Review cost visibility without requiring credentials or a real provider call.", "/ai-costing", [_step("OPEN_AI_COSTING", "/ai-costing", "Open the AI Costing dashboard.", "Real model default, key presence, usage, and estimated cost are visible.", "Cost summary."), _step("OPEN_MODEL_PRICING", "/ai-costing/models", "Open the model pricing table.", "OpenAI models and editable local assumptions are visible.", "Pricing table."), _step("UPDATE_PRICING", "/ai-costing/models", "Update a placeholder price only when permitted.", "Non-negative pricing is saved as a local assumption and no model call occurs.", "Pricing note and effective date.", True, "Pricing mutation changes local governance data only."), _step("OPEN_SMOKE_TEST", "/ai-costing/smoke-test", "Open the one-shot smoke-test page.", "Model selector, readiness, acknowledgement, and dry-run controls are visible.", "Smoke controls."), _step("DRY_RUN_SMOKE", "/ai-costing/smoke-test", "Click Dry run.", "Maximum cost and readiness reasons are shown; no external call occurs.", "Dry-run result."), _step("VERIFY_RUN_GUARD", "/ai-costing/smoke-test", "Leave real model disabled and inspect the run button.", "A real run is unavailable until every explicit prerequisite is satisfied.", "Disabled run button."), _step("OPEN_USAGE", "/ai-costing/usage", "Open usage metering.", "Token counts, source, model, and estimated cost are visible when records exist.", "Usage table.")])]},
    {"suite_code": "STAGE3_AUTONOMOUS_SANDBOX_VALIDATION", "title": "Stage 3 Autonomous Sandbox Validation", "description": "Validate the disabled-by-default, bounded, dry-run-first local autonomy sandbox.", "experience": "agentic,operations,full", "sort_order": 78, "cases": [_case("STAGE3_SANDBOX_FLOW", "Local sandbox governance flow", "Validate Stage 3 planning and safety boundaries without requiring execution.", "/stage3-autonomy", [_step("OPEN_STAGE3_DASHBOARD", "/stage3-autonomy", "Open the Stage 3 sandbox dashboard.", "Sandbox disabled-by-default status, limits, and safety warnings are visible.", "Status and limits."), _step("VERIFY_STAGE3_PROFILES", "/stage3-autonomy/profiles", "Review the bounded profiles.", "Profiles expose only predefined local safe actions.", "Profile action lists."), _step("CREATE_STAGE3_RUN", "/stage3-autonomy/runs", "Create a dry-run-only sandbox request with the sandbox acknowledgement.", "A run is created without executing an action.", "Run ID and status.", True, "This creates only local sandbox audit records."), _step("RUN_STAGE3_DRY_RUN", "/stage3-autonomy/runs/<RUN_ID>", "Click Dry Run.", "Planned safe local actions and human handback conditions are displayed; executions remain zero.", "Dry-run plan."), _step("VERIFY_EXECUTION_GUARD", "/stage3-autonomy/runs/<RUN_ID>", "Inspect execution controls while the sandbox is disabled.", "Start is blocked unless configuration, dry-run, and acknowledgements all pass.", "Blocked execution state."), _step("VERIFY_STAGE3_AUDIT", "/stage3-autonomy/runs/<RUN_ID>", "Review steps and events.", "Run creation, policy checks, and dry-run events are auditable.", "Event timeline."), _step("VERIFY_NO_EXTERNALS", "/stage3-autonomy", "Review the safety warning.", "No shell, SQL, external system, ServiceNow, customer-send, or production remediation capability is presented.", "Safety boundary.")])]},
...
def seed_catalog(db: Session) -> int:
    created = 0
    # Stage 3 is intentionally folded into the existing governance suite so
    # older acceptance reports retain their suite taxonomy while still
    # covering the new sandbox boundary.
    catalog_data = [item for item in UI_TEST_CATALOG if item["suite_code"] != "STAGE3_AUTONOMOUS_SANDBOX_VALIDATION"]
    governance = next(item for item in catalog_data if item["suite_code"] == "GOVERNANCE_BOUNDARY_VALIDATION")
    governance_case = governance["cases"][0]
    if not any(item["step_code"] == "VERIFY_STAGE3_SANDBOX" for item in governance_case["steps"]):
        governance_case["steps"].extend([_step("VERIFY_STAGE3_SANDBOX", "/stage3-autonomy", "Open the Stage 3 sandbox status.", "The sandbox is disabled by default, bounded, dry-run-first, and local-only.", "Stage 3 status and safety boundary."), _step("VERIFY_STAGE3_POLICY", "/stage3-autonomy/profiles", "Review Stage 3 profiles and permitted actions.", "Only predefined local safe actions are listed; no external execution is available.", "Profile policy."), _step("VERIFY_STAGE3_AUDIT", "/stage3-autonomy/runs/<RUN_ID>", "Review a sandbox dry-run when available.", "Run, guardrail, and handback events are visible without requiring execution.", "Sandbox audit events.")])
    if not any(item["step_code"] == "VERIFY_BASELINE_COMPLETION" for item in governance_case["steps"]):
        governance_case["steps"].extend([_step("VERIFY_BASELINE_COMPLETION", "/baseline-completion", "Open Baseline Completion.", "Baseline status, completed prompt range, and read-only safety boundaries are visible.", "Baseline dashboard."), _step("VERIFY_REQUIREMENTS_TRACEABILITY", "/baseline-completion/requirements", "Open the requirements matrix.", "Covered and explicitly out-of-scope requirements include route and test evidence.", "Traceability matrix."), _step("VERIFY_HANDOVER_PACK", "/baseline-completion/handover", "Open the generated handover pack.", "Walkthroughs, stage modes, limitations, and sign-off content are available.", "Handover pack.")])
    for suite_data in catalog_data:
    ...
    # The Stage 3 governance checks are part of the stable governance suite;
    # archive an earlier development-only standalone row if one exists.
    legacy_stage3 = db.scalar(select(UiTestSuite).where(UiTestSuite.suite_code == "STAGE3_AUTONOMOUS_SANDBOX_VALIDATION"))
    if legacy_stage3 is not None:
        legacy_stage3.is_enabled = False
        for legacy_case in db.scalars(select(UiTestCase).where(UiTestCase.suite_code == legacy_stage3.suite_code)).all():
            legacy_case.is_enabled = False
    db.flush()
    return created
...
def list_suites(db: Session) -> list[dict[str, Any]]:
    suites = db.scalars(select(UiTestSuite).where(UiTestSuite.is_enabled.is_(True)).order_by(UiTestSuite.sort_order, UiTestSuite.suite_code)).all()
